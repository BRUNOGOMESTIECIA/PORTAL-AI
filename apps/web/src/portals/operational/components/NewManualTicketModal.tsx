...
import { useDynamicQueues } from '../pages/admin/QueuesManagerPanel';
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      const next = !isCustomTeam;
                      setIsCustomTeam(next);
                      if (next) {
                        setValue('team', customTeamName.trim());
                      } else {
                        setValue('team', '');
                      }
                    }}
                  ...
                  >
                    {isCustomTeam ? '← Escolher da lista' : '+ Digitar nova mesa'}
                  </button>
                </div>
                ...
                {isCustomTeam ? (
                  <input
                    type="text"
                    placeholder="Nome da mesa (ex: N2, VIP, Redes)..."
                    value={customTeamName}
                    onChange={(e) => {
                      setCustomTeamName(e.target.value);
                      setValue('team', e.target.value.trim());
                    }}
                    className="w-full rounded-xl border border-blue-300 dark:border-blue-700 bg-blue-50/50 dark:bg-blue-950/20 px-4 py-2.5 text-sm outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-50 text-slate-900 dark:text-slate-100 font-medium"
                    autoFocus
                  />
                ) : (
                  <div className="relative">
                    ...
                    >
                      ...
                      {Array.from(new Set([
                        ...TEAMS,
                        ...queues.filter(q => q.isActive).map(q => q.name.split(' - ')[0].trim()),
                        ...queues.filter(q => q.isActive).map(q => q.name),
