...
export default function LiveInboxPage() {
  ...
  const [chats, setChats] = useState<ChatSession[]>([
    ...
    },
    {
      ...
      customerRole: "Jornalista Sênior",
    ...
    },
    {
      ...
      customerRole: "Embaixadora",
      tenantName: "Themyscira Org",
      tenantPlan: "Enterprise",
      mrr: "$20.000",
      status: "active",
      ...
      isViolatingSLA: true,
      messages: [
        { id: 1, sender: "customer", name: "Diana Prince", time: "11:00", content: "O relatório de métricas está demorando muito para carregar." },
        { id: 2, sender: "agent", name: "João Técnico", time: "11:05", content: "Estou verificando isso com o time de engenharia, Diana." }
      ]
    }
  ]);

  const filteredChats = chats.filter(c => {
    if (filterTab === "entrada") return c.status === "waiting";
    if (filterTab === "meus") return c.assignee === "Você" && c.status === "active";
    if (filterTab === "em_atendimento") return c.status === "active"; // Todos ativos
    if (filterTab === "finalizado") return c.status === "resolved";
    return false;
  });
  ...
  const openCheckoutModal = () => {
    if (activeChat) {
      setCheckoutSummary(`O cliente (${activeChat.customerName}) enfrentou problemas na integração do webhook, indicando erro 500 no painel. O problema foi diagnosticado como um rate-limit ativado no servidor de destino. A chave da API foi renovada e o webhook voltou a operar em tempo real.`);
    }
  ...
  const handleTransferChat = (newAssignee: string) => {
    if (!activeChat) return;
    setChats(chats.map(c => c.id === activeChatId ? { ...c, queueStatus: "em_atendimento", assignee: newAssignee } : c));
    setShowTransferModal(false);
    toast.success(`Chat transferido com sucesso para ${newAssignee}.`);
    
    // Se o técnico transferiu, tira da aba 'meus'
    if (userRole === "tecnico") {
      const nextChat = chats.find(c => c.id !== activeChatId && c.assignee === "Você");
      setActiveChatId(nextChat ? nextChat.id : "");
    }
  };
  ...
  return (
    <div className="flex h-[calc(100vh-5rem)] bg-[#050505] overflow-hidden text-sm">
            ...
            >
               ...
               <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                     ...
                     {filterTab === "em_atendimento" && chat.assignee && (
                       <span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-blue-500/20 text-blue-400 border border-blue-500/30 truncate max-w-[80px]">
                         {chat.assignee}
                       </span>
                     )}
                  </div>
      ...
      {activeChat && (
        <div className="w-72 bg-[#0A0A0A] border-l border-gray-800 flex flex-col flex-shrink-0 z-20 shadow-2xl">
           ...
           <div className="flex-1 overflow-y-auto p-5 space-y-6 custom-scrollbar">
              ...
              {/* Action Buttons (Assumir vs Transferir vs Encerrar) */}
              {activeChat.status === "waiting" ? (
                ...
                <div className="flex flex-col gap-3">
                  {(userRole === "admin" || activeChat.assignee === "Você") && (
                    <button 
                      onClick={() => setShowTransferModal(true)}
                      className="w-full py-3 bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 border border-blue-500/30 rounded-xl font-bold transition-all flex items-center justify-center gap-2 shadow-sm"
                    >
                       Transferir Chat
                    </button>
                  )}
                  
                  {activeChat.assignee !== "Você" && userRole === "admin" && (
                     <button 
                       onClick={() => setShowTakeoverModal(true)}
                       className="w-full py-3 bg-purple-600/20 hover:bg-purple-600/30 text-purple-400 border border-purple-500/30 rounded-xl font-bold transition-all flex items-center justify-center gap-2"
                     >
                        Puxar Atendimento para Mim
                     </button>
                  )}
                 ...
                 <div className="p-4 rounded-xl bg-[#111111] border border-gray-800">
                   ...
                   <div className="space-y-3 pt-4 border-t border-gray-800/50">
                     <div className="flex justify-between items-center text-xs">
                       ...
                       <span className="font-bold text-gray-300">{activeChat.customerRole}</span>
                     </div>
                     <div className="flex justify-between items-center text-xs">
           ...
           <div className="bg-[#111111] border border-gray-800 rounded-3xl p-8 max-w-md w-full shadow-2xl relative animate-in zoom-in-95">
              ...
              <div className="mb-6 bg-purple-500/10 border border-purple-500/20 rounded-xl p-4">
                 <h3 className="text-xs font-bold text-purple-400 mb-2 flex items-center gap-2">
                   <SparklesIcon className="w-4 h-4" />
                   Resumo do Atendimento (Editável)
                 </h3>
                 <textarea 
                   className="w-full h-24 bg-transparent border-none text-gray-200 text-sm focus:outline-none resize-none custom-scrollbar"
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                 <div className="bg-[#0A0A0A] border border-gray-800 rounded-xl p-4">
                    <h3 className="text-xs font-bold text-gray-500 mb-2">Predição de CSAT (IA)</h3>
                    <div className="flex items-center gap-1 text-2xl">
                       ⭐ ⭐ ⭐ ⭐ <span className="opacity-30">⭐</span> <span className="text-sm font-bold text-white ml-2">4.0</span>
                    </div>
                 </div>
                 <div className="bg-[#0A0A0A] border border-gray-800 rounded-xl p-4">
                    <h3 className="text-xs font-bold text-gray-500 mb-2">Tags Sugeridas</h3>
                    <div className="flex flex-wrap gap-2">
                       <span className="px-2 py-1 bg-red-500/10 text-red-400 text-[10px] font-bold rounded border border-red-500/20">Bug</span>
                       <span className="px-2 py-1 bg-blue-500/10 text-blue-400 text-[10px] font-bold rounded border border-blue-500/20">API Webhook</span>
                       <button className="px-2 py-1 bg-gray-800 text-gray-400 hover:text-white text-[10px] font-bold rounded border border-gray-700">+</button>
                    </div>
                 </div>
              </div>
              ...
              <div className="space-y-3 mb-8">
                 <label className="flex items-center gap-3 text-sm text-gray-300 cursor-pointer">
                   <input type="checkbox" defaultChecked className="w-4 h-4 rounded border-gray-700 bg-gray-900 text-purple-600 focus:ring-purple-600 focus:ring-offset-gray-900" />
                   Enviar transcrição do chat por e-mail para o cliente
                 </label>
                 <label className="flex items-center gap-3 text-sm text-gray-300 cursor-pointer">
                   <input type="checkbox" className="w-4 h-4 rounded border-gray-700 bg-gray-900 text-purple-600 focus:ring-purple-600 focus:ring-offset-gray-900" />
                   Criar tarefa de Follow-up (Revisar em 3 dias)
                 </label>
              </div>
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar space-y-4 pr-2">
                 <div className="bg-blue-500/5 border border-blue-500/20 rounded-xl p-4">
                    ...
                    <div className="mt-4 pt-4 border-t border-blue-500/20 flex items-center justify-between">
                      <span className="text-[10px] font-bold text-blue-500/50 uppercase">Esta sugestão foi útil?</span>
                      <div className="flex gap-2">
                        <button className="p-1.5 bg-gray-800 hover:bg-gray-700 text-gray-400 hover:text-green-400 rounded transition-colors"><HandThumbUpIcon className="w-4 h-4" /></button>
                        <button className="p-1.5 bg-gray-800 hover:bg-gray-700 text-gray-400 hover:text-red-400 rounded transition-colors"><HandThumbDownIcon className="w-4 h-4" /></button>
                      </div>
                    </div>
                 </div>
      ...
      {showTransferModal && activeChat && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in">
           <div className="bg-[#111111] border border-gray-800 rounded-3xl p-6 max-w-sm w-full shadow-2xl relative animate-in zoom-in-95">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-lg font-bold text-white">Transferir Chat</h2>
                <button onClick={() => setShowTransferModal(false)} className="text-gray-500 hover:text-white">
                   <XMarkIcon className="w-6 h-6" />
                </button>
              </div>
              <p className="text-xs text-gray-400 mb-4">Selecione um técnico online para assumir o atendimento de <strong>{activeChat.customerName}</strong>:</p>
              
              <div className="space-y-2">
                 <button onClick={() => handleTransferChat("João Técnico")} className="w-full p-3 bg-[#0A0A0A] border border-gray-800 hover:border-blue-500/50 rounded-xl flex items-center gap-3 transition-colors text-left group">
                    <div className="w-8 h-8 rounded-full bg-blue-500/20 flex items-center justify-center text-blue-400 font-bold text-xs relative">
                       JT
                       <span className="absolute bottom-0 right-0 w-2 h-2 bg-green-500 border border-[#0A0A0A] rounded-full"></span>
                    </div>
                    <div>
                      <div className="text-sm font-bold text-gray-200 group-hover:text-white">João Técnico</div>
                      <div className="text-[10px] text-gray-500">2 chats em andamento</div>
                    </div>
                 </button>

                 <button onClick={() => handleTransferChat("Maria Suporte")} className="w-full p-3 bg-[#0A0A0A] border border-gray-800 hover:border-blue-500/50 rounded-xl flex items-center gap-3 transition-colors text-left group">
                    <div className="w-8 h-8 rounded-full bg-fuchsia-500/20 flex items-center justify-center text-fuchsia-400 font-bold text-xs relative">
                       MS
                       <span className="absolute bottom-0 right-0 w-2 h-2 bg-green-500 border border-[#0A0A0A] rounded-full"></span>
                    </div>
                    <div>
                      <div className="text-sm font-bold text-gray-200 group-hover:text-white">Maria Suporte</div>
                      <div className="text-[10px] text-gray-500">Ociosa</div>
                    </div>
                 </button>
              </div>
           </div>
      ...
      {/* ------------------------------------------------------------- */}
      {showTakeoverModal && activeChat && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in">
           <div className="bg-[#111111] border border-gray-800 rounded-3xl p-6 max-w-sm w-full shadow-2xl relative animate-in zoom-in-95 text-center">
              <div className="w-16 h-16 bg-purple-500/10 rounded-full flex items-center justify-center border border-purple-500/30 mx-auto mb-4">
                 <SparklesIcon className="w-8 h-8 text-purple-400" />
              </div>
              <h2 className="text-lg font-bold text-white mb-2">Puxar Atendimento</h2>
              <p className="text-sm text-gray-400 mb-6 leading-relaxed">
                 O técnico <strong>{activeChat.assignee}</strong> está atualmente neste chat. Tem certeza que deseja assumir o controle? Ele será notificado.
              </p>
              
              <div className="flex gap-3">
                 <button onClick={() => setShowTakeoverModal(false)} className="flex-1 py-3 bg-gray-800 hover:bg-gray-700 text-white rounded-xl font-bold transition-colors text-sm">
                    Cancelar
                 </button>
                 <button onClick={handleAssumirAtendimento} className="flex-1 py-3 bg-purple-600 hover:bg-purple-500 text-white rounded-xl font-bold shadow-[0_0_15px_rgba(147,51,234,0.3)] transition-all text-sm">
                    Sim, Assumir
                 </button>
              </div>
           </div>
